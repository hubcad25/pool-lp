# Age Curves V1: Facteur global par âge
# Approche: Même facteur d'âge pour tous les features

# Packages ---------------------------------------------------------------
library(dplyr)
library(tidyr)

cat("\n=== Age Curves V1: Facteur Global ===\n\n")
...
cat("Construction des courbes d'âge globales...\n")
...
# Calculer facteurs d'âge par feature -----------------------------------
...
# Calculer facteur GLOBAL par âge ----------------------------------------
cat("Calcul des facteurs globaux (moyenne across features)...\n")

global_age_factors <- data.frame()

for (age_val in 19:40) {
  for (pos in c("F", "D")) {
    factors_at_age <- sapply(features_to_project, function(feat) {
      af <- age_factors[[feat]]
      factor_row <- af %>% filter(position_group == pos, age == age_val)
      if (nrow(factor_row) > 0) {
        return(factor_row$age_factor)
      } else {
        return(NA)
      }
    })

    global_factor <- mean(factors_at_age, na.rm = TRUE)

    if (is.na(global_factor) || !is.finite(global_factor)) {
      global_factor <- 1.0
    }

    global_age_factors <- rbind(
      global_age_factors,
      data.frame(
        position_group = pos,
        age = age_val,
        global_age_factor = global_factor
      )
    )
  }
}
...
# Générer prédictions ----------------------------------------------------
cat("Génération des prédictions avec facteur global...\n")
...
for (feature in features_to_project) {
  ...
  # Appliquer facteur global
  preds <- df_valid_targets %>%
    select(player_id, age, position) %>%
    mutate(position_group = ifelse(position %in% c("C", "L", "R"), "F", "D")) %>%
    left_join(wma_base, by = "player_id") %>%
    left_join(global_age_factors, by = c("position_group", "age")) %>%
    mutate(
      global_age_factor = ifelse(is.na(global_age_factor), 1.0, global_age_factor),
      pred_value = wma_value * global_age_factor
    )
...
# Afficher résultats -----------------------------------------------------
cat("\n=== Résultats Age Curves V1 ===\n\n")
print(metrics, row.names = FALSE, digits = 3)

# Sauvegarder ------------------------------------------------------------
dir.create("data/01_point_projections/projection/experiments/results/age_curves_v1",
           showWarnings = FALSE, recursive = TRUE)

saveRDS(metrics, "data/01_point_projections/projection/experiments/results/age_curves_v1/metrics.rds")
saveRDS(predictions, "data/01_point_projections/projection/experiments/results/age_curves_v1/predictions.rds")
saveRDS(global_age_factors, "data/01_point_projections/projection/experiments/results/age_curves_v1/global_age_factors.rds")

cat("\n✓ Résultats sauvegardés dans data/.../results/age_curves_v1/\n\n")
