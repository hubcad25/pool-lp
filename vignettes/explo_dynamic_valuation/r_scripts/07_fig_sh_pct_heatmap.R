...
# Heatmap: Joueurs × Temps (Écart L10 - cumulatif SH%)
...
# Heatmap
fig_sh_pct_heatmap <- ggplot(df_heatmap, aes(x = game_index, y = player_name, fill = diff_L10_cumul)) +
  geom_tile() +
  ...
  labs(
    ...
    subtitle = "Écart entre Rolling 10 matchs et Cumulatif (SH%)",
    x = "Match #",
    y = NULL,
    caption = "Rouge = Hot streak (>0), Bleu = Cold streak (<0)"
  ) +
  theme_minimal() +
  theme(
