# Script: 08_fig_sh_pct_spaghetti.R
# Spaghetti plot: Tous les joueurs + Percentiles population
...
fig_sh_pct_spaghetti <- ggplot() +
  ...
  geom_hline(yintercept = c(-5, 5), linetype = "dashed", color = "gray40", alpha = 0.5) +
  # Percentiles (bold)
  geom_line(data = df_percentiles, aes(x = game_index, y = P10), color = "#3498db", linewidth = 1, alpha = 0.8) +
  geom_line(data = df_percentiles, aes(x = game_index, y = P50), color = "#2c3e50", linewidth = 1.2) +
  geom_line(data = df_percentiles, aes(x = game_index, y = P90), color = "#e74c3c", linewidth = 1, alpha = 0.8) +
  # Bande interquartile
  geom_ribbon(
    data = df_percentiles,
