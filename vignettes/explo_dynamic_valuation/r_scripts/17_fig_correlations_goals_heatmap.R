# Script 17: Heatmap de corrélations - Goals (multi-fenêtres)
# Objectif: Montrer corrélations entre features de chance et production future de goals
...
calc_cor_matrix <- function(df, horizon) {
  targets <- c(
    ...
    paste0("excess_goals_next_", horizon)
  )
  ...
  cor_long <- as.data.frame(cor_matrix) %>%
    ...
    mutate(
      ...
      Target_label = case_when(
        Target == paste0("goals_next_", horizon) ~ paste0("Goals (", horizon, " matchs)"),
        Target == paste0("excess_goals_next_", horizon) ~ paste0("Excess goals (", horizon, " matchs)")
      )
    )
...
fig_correlations_goals_F <- ggplot(cor_results_F, aes(x = Target_label, y = Feature_label, fill = Correlation)) +
  ...
  labs(
    title = "Corrélations: Features de Chance → Production de Goals (Forwards)",
...
fig_correlations_goals_D <- ggplot(cor_results_D, aes(x = Target_label, y = Feature_label, fill = Correlation)) +
  geom_tile(color = "white", linewidth = 0.5) +
  geom_text(aes(label = sprintf("%.2f", Correlation)), color = "white", size = 3, fontface = "bold") +
  facet_wrap(~ paste0("Horizon: ", Horizon, " matchs"), ncol = 4, scales = "free_x") +
  scale_fill_gradient2(
  ...
  labs(
    title = "Corrélations: Features de Chance → Production de Goals (Defensemen)",
    subtitle = "Corrélations négatives (bleu) = régression attendue | Positives (rouge) = signal de talent",
    x = NULL,
    y = "Features de chance (ordonnées alphabétiquement)",
